...
export async function analyzeImage(input: ImageAnalysisInput): Promise<AIAnalysisResult> {
  const customEndpoint = process.env.NEXT_PUBLIC_AI_VISION_ENDPOINT;
  const apiEndpoint = customEndpoint || "/api/analyze-image";

  // 1. Send request to Server-side Gemini Vision API route
  try {
    ...
    if (response.ok) {
      ...
      const isGoodStatus = data.status === "GOOD" || !data.issueDetected;
      const damageDetected = !isGoodStatus && Boolean(data.issueDetected);
      const severity = damageDetected && typeof data.severity === "number" ? data.severity : (isGoodStatus ? data.severity || 0 : null);
      const priority = damageDetected && severity !== null ? (data.priority || calculatePriorityFromSeverity(severity, data.category)) : null;

      const detectedObject = isGoodStatus
        ? "Normal / Maintained Infrastructure"
        : (data.category || "Damaged Infrastructure");

      const deptMap: Record<string, string> = {
        "Road & Pavement": "Department of Roads & Infrastructure",
        "Water Infrastructure": "Water Supply & Sewerage Board",
        "Sanitation & Waste": "Department of Sanitation & Waste",
        "Streetlighting": "City Energy & Public Lighting",
        "Drainage": "Water Supply & Sewerage Board",
        "Public Safety": "City Emergency & Public Safety",
      };

      return {
        isReportable: damageDetected,
        detectedObject,
        issueType: damageDetected ? (data.category || "Road Damage") : null,
        damageDetected,
        severity: damageDetected ? severity : null,
        confidence: typeof data.confidence === "number" ? data.confidence : 92,
        priority,
        explanation: data.assessment || (damageDetected ? "Defect identified in municipal infrastructure." : "Infrastructure appears in good condition."),
        suggestedDepartment: damageDetected ? (deptMap[data.category] || "Department of Roads & Infrastructure") : null,
        slaHours: calculateSLAHours(priority),
        isModelConnected: true,
        serviceStatus: "CONNECTED",
        modelName: "Gemini 1.5 Flash Vision (Google AI)",
      };
    } else {
      const errorJson = await response.json().catch(() => null);
      console.warn("Gemini Vision API endpoint returned error:", response.status, errorJson);
    }
  } catch (err) {
    console.warn("AI Model Endpoint unreachable, falling back to heuristics:", err);
  }

  // 2. Fallback Heuristics for offline/demo environments
